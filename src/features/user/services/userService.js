      ...
      if (user) {
        ...
        if (db) {
          try {
            ...
            // Silently handle permission errors during initialization
            if (error.code !== 'permission-denied') {
              handleFirestoreError(error);
            }
            // Continue with authentication even if Firestore fails
            console.warn('Firestore initialization skipped - please update security rules');
          }
        }
      ...
      if (docSnapshot.exists()) {
        ...
        chunks.forEach(chunk => {
          ...
          const unsubscribeFriends = onSnapshot(q, (querySnapshot) => {
            ...
            // Don't clear friends list on permission errors - just log it
            if (error.code !== 'permission-denied') {
              console.error('Friends subscription error:', error)
              callback([])
            } else {
              console.warn('Firestore permission error - update security rules to see friends list')
            }
          })
        ...
        // Don't call callback with empty array to avoid clearing chat
        return () => {}
      }
    }, (error) => {
      // Don't clear friends list on permission errors
      if (error.code !== 'permission-denied') {
        console.error('User document subscription error:', error)
        callback([])
      } else {
        console.warn('Firestore permission error - update security rules')
      }
      return () => {}
    })
