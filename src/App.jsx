...
import { X, Phone, MessageCircle, User, Home, Users, Bell, Settings } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading TalkMe...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background">
        <header className="border-b">
          <div className="container flex items-center justify-between h-16 px-4">
            <div className="flex items-center gap-2">
              <img src="/talkme-logo.png" alt="TalkMe" className="w-8 h-8 rounded-full" />
        ...
        <main className="container max-w-4xl mx-auto p-4">
          <div className="bg-card rounded-xl border shadow-sm p-6 sm:p-8">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-2">Welcome to TalkMe</h2>
  ...
  return (
    <OnlineStatusProvider>
      <ThemeContext.Provider value={{ theme, toggleTheme }}>
        <div className="min-h-screen bg-background">
          {/* Header - Premium Modern Navigation */}
            ...
            <div className="container mx-auto px-6 lg:px-8">
              <div className="flex items-center justify-between h-20">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-2xl font-bold bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent tracking-tight" style={{fontFamily: 'Inter, sans-serif'}}>TalkMe</h1>
                </div>
                
                {/* Center Navigation - Desktop Only */}
                <nav className="hidden lg:flex items-center gap-2 bg-secondary/30 rounded-2xl p-1.5 border border-border/30">
                  <button
                    aria-label="Home"
                    className="relative flex items-center justify-center h-11 px-5 rounded-xl text-muted-foreground transition-all duration-300 hover:text-foreground hover:bg-background/80 hover:shadow-md group"
                  >
                    <Home className="h-5 w-5" strokeWidth={2} />
                    <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-foreground text-background text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">Home</span>
                  </button>
                  <button
                    aria-label="Chats"
                    className="relative flex items-center justify-center h-11 px-5 rounded-xl bg-primary text-primary-foreground transition-all duration-300 shadow-lg shadow-primary/30 hover:shadow-xl hover:shadow-primary/40 group"
                  >
                    <MessageCircle className="h-5 w-5" strokeWidth={2} />
                    <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-foreground text-background text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">Chats</span>
                  </button>
                  <button
                    aria-label="People"
                    className="relative flex items-center justify-center h-11 px-5 rounded-xl text-muted-foreground transition-all duration-300 hover:text-foreground hover:bg-background/80 hover:shadow-md group"
                  >
                    ...
                    <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-foreground text-background text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">People</span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <div className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full ring-2 ring-card"></div>
                    <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-foreground text-background text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">Notifications</span>
                  </button>
                </nav>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    aria-label="Search"
                    className="hidden md:flex items-center justify-center h-11 px-4 rounded-xl bg-secondary/50 border border-border/30 text-muted-foreground transition-all duration-300 hover:bg-secondary hover:border-border/60 hover:text-foreground hover:shadow-md group"
                  >
                  ...
                  <button
                    aria-label="Settings"
                    className="hidden md:flex items-center justify-center h-11 px-4 rounded-xl bg-secondary/50 border border-border/30 text-muted-foreground transition-all duration-300 hover:bg-secondary hover:border-border/60 hover:text-foreground hover:shadow-md hover:rotate-90 group"
                  >
                    <Settings className="h-5 w-5 transition-transform duration-300" strokeWidth={2} />
                  </button>
                  ...
                  <button
                    ...
                    aria-label="Toggle theme">
                    {theme === 'dark' ? (
                      <Sun className="h-5 w-5 text-amber-500 group-hover:rotate-180 transition-transform duration-500" strokeWidth={2} />
                    ) : (
                      <Moon className="h-5 w-5 text-indigo-600 group-hover:-rotate-12 transition-transform duration-500" strokeWidth={2} />
                    )}
                  </button>
                  
                  {/* User Profile */}
                  <div className="hidden sm:flex items-center gap-3 pl-3 border-l border-border/30">
                    <div className="text-right">
                      <p className="text-sm font-medium text-foreground" style={{fontFamily: 'Inter, sans-serif'}}>{user.displayName?.split(' ')[0] || 'User'}</p>
                      <p className="text-xs text-muted-foreground">Online</p>
                    </div>
                    <div className="relative">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-primary/60 flex items-center justify-center text-white font-semibold shadow-lg shadow-primary/20 ring-2 ring-background">
          ...
          <div className={`
            ...
            ${screenSize.isTablet ? 'grid-cols-[1fr] gap-4 p-4' : ''}
            ${screenSize.isDesktop ? 'grid-cols-[350px_1fr_350px] gap-6 p-6' : ''}
            ${screenSize.isDesktop ? 'h-[90vh]' : ''}
            ${screenSize.isLargeScreen ? 'grid-cols-[400px_1fr_400px] gap-8 p-8' : ''}
            ${screenSize.isFoldable ? 'flex flex-row h-[90vh]' : ''}
          `}>
            {/* Conversations list - hidden on mobile, shown on tablet/desktop */}
            {!screenSize.isMobile && (
              <aside className={screenSize.isFoldable ? 'w-1/3' : ''}>
